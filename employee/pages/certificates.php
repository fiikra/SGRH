...
// Récupérer les attestations de l'employé
$certificates = $db->prepare("SELECT * FROM certificates 
                            WHERE employee_nin = ? 
                            ORDER BY issue_date DESC");
$certificates->execute([$employee['nin']]);

$pageTitle = "Mes Attestations";
include $_SERVER['DOCUMENT_ROOT'] .'/includes/header.php'; 
?>

<div class="container">
    <h1 class="mb-4">Mes Attestations</h1>
    ...
    <div class="card">
        <div class="card-body">
            <?php if ($certificates->rowCount() > 0): ?>
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php while ($cert = $certificates->fetch()): ?>
                                <tr>
                                    <td>
                                        <?= $cert['certificate_type'] === 'work' ? 'Attestation de Travail' : 
                                           ($cert['certificate_type'] === 'salary' ? 'Attestation de Salaire' : 'Certificat d\'Emploi') ?>
                                    </td>
            ...
            <?php else: ?>
                <div class="alert alert-info">Aucune attestation trouvée.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
